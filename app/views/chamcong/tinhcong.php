    ...
    <div class="dashboard-container">
        ...
        <div class="panel payroll-toolbar">
                <div class="payroll-metrics">
                    ...
                    <div class="payroll-metric" style="border-color:#fed7aa;background:linear-gradient(135deg,#fff7ed 0%,#fff 80%);">
                        <span class="payroll-metric-label" style="color:#92400e;">Chấm công hộ (HR)</span>
                        <strong id="metric-override" style="color:#d97706;"><?= (int)$overrideCount ?> lần</strong>
                        <div style="margin-top:8px;">
                            <button id="btn-open-override-modal" class="btn btn-sm" style="background:#f59e0b;color:#fff;border:none;border-radius:10px;padding:6px 14px;font-size:.82rem;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">
                                <i class="fas fa-plus"></i> Chấm hộ
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="panel payroll-board">
                <div class="payroll-board-head">
                    <div class="sub-tabs">
                        ...
                        <button class="sub-tab" data-tab="tab-chamconghothay" id="tab-btn-override"><i class="fas fa-user-shield"></i> Chấm Công Hộ (HR) <span id="override-tab-badge" style="background:#f59e0b;color:#fff;border-radius:20px;padding:1px 8px;font-size:.76rem;margin-left:4px;"><?= (int)$overrideCount ?></span></button>
                    </div>
                ...
                <div class="payroll-board-body">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <button type="button" class="btn btn-info btn-xs"
                                                    onclick="showEmployeeDetailById(<?= (int)($row['maND'] ?? 0) ?>, '<?= htmlspecialchars($selectedMonth, ENT_QUOTES) ?>')">
                    ...
                    <!-- TAB: CHẤM CÔNG HỘ (HR) -->
                    <div class="tab-content" id="tab-chamconghothay">
                        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:16px;">
                            <div>
                                <h3 style="margin:0 0 4px;">Lịch sử chấm công hộ - Tháng <span id="override-month-label"><?= htmlspecialchars($selectedMonth) ?></span></h3>
                                <p style="margin:0;color:#64748b;font-size:.88rem;">Mỗi lần chấm hộ đều được ghi nhận đầy đủ để đảm bảo HR không lạm quyền.</p>
                            </div>
                            <button type="button" id="btn-open-override-modal-2" class="btn" style="background:#f59e0b;color:#fff;border:none;border-radius:12px;padding:10px 20px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:8px;">
                                <i class="fas fa-plus"></i> + Chấm công hộ bởi HR
                            </button>
                        </div>
                        <div id="override-history-loading" style="text-align:center;padding:40px;color:#94a3b8;display:none;">
                            <i class="fas fa-spinner fa-spin fa-2x"></i><br><span style="margin-top:8px;display:block;">Đang tải dữ liệu...</span>
                        </div>
                        <div style="overflow-x:auto;">
                            <table class="override-history-table" id="override-history-table">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>Nhân viên</th>
                                ...
                                <tbody id="override-history-body">
                                    <tr><td colspan="9" class="empty-state">Chọn tab để tải dữ liệu...</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
...
<!-- MODAL: CHẤM CÔNG HỘ (HR) - 3 bước -->
<div class="override-modal" id="hrOverrideModal" aria-hidden="true">
    <div class="override-card">
        <div class="override-card-head">
            <div>
                <h3><i class="fas fa-user-shield" style="margin-right:8px;"></i>Chấm Công Hộ bởi HR</h3>
                <p>Ghi nhận ngày công hộ khi nhân viên gặp sự cố. Mọi thao tác đều được lưu audit trail.</p>
            </div>
            <button type="button" class="override-close" id="override-modal-close" aria-label="Đóng">×</button>
        </div>
        <div class="override-steps">
            <button type="button" class="override-step-btn active" data-step="1">
                <span class="step-num">1</span> Chọn nhân viên
            </button>
            <button type="button" class="override-step-btn" data-step="2">
                <span class="step-num">2</span> Ngày &amp; lý do
            </button>
            <button type="button" class="override-step-btn" data-step="3">
                <span class="step-num">3</span> Xác nhận
            </button>
        </div>
        <div class="override-body">
            <!-- Bước 1: Chọn nhân viên -->
            <div class="override-step-pane active" id="override-pane-1">
                <label style="font-weight:700;color:#374151;display:block;margin-bottom:8px;">Tìm kiếm nhân viên <span style="color:#ef4444">*</span></label>
                <select id="override-nv-select" style="width:100%;padding:10px 14px;border:1px solid #d1d5db;border-radius:10px;font-size:.95rem;outline:none;">
                    <option value="">-- Chọn nhân viên --</option>
                    <?php foreach ($filterEmployees as $emp): ?>
                    <option value="<?= (int)$emp['maND'] ?>" data-name="<?= htmlspecialchars($emp['hoTen']) ?>" data-dept="<?= htmlspecialchars($emp['phongBan'] ?? '') ?>">
                        #<?= (int)$emp['maND'] ?> - <?= htmlspecialchars($emp['hoTen']) ?> (<?= htmlspecialchars($emp['phongBan'] ?? 'N/A') ?>)
                    </option>
                    <?php endforeach; ?>
                </select>
                <div id="override-nv-preview" style="margin-top:14px;display:none;">
                    <div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:#fff7ed;border:1px solid #fed7aa;border-radius:12px;">
                        ...
                        <div>
                            <div id="override-nv-name" style="font-weight:700;color:#92400e;font-size:1rem;"></div>
                            <div id="override-nv-dept" style="color:#d97706;font-size:.85rem;"></div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Bước 2: Chọn ngày & lý do -->
            <div class="override-step-pane" id="override-pane-2">
                <div style="display:grid;gap:16px;">
                    <div>
                        <label style="font-weight:700;color:#374151;display:block;margin-bottom:6px;">Ngày chấm hộ <span style="color:#ef4444">*</span></label>
                        <input type="date" id="override-ngay" max="<?= date('Y-m-d') ?>"
                            style="width:100%;padding:10px 14px;border:1px solid #d1d5db;border-radius:10px;font-size:.95rem;">
                        <p style="margin:6px 0 0;color:#64748b;font-size:.82rem;">Chỉ được chấm cho ngày hiện tại hoặc ngày quá khứ (không thể chấm ngày tương lai).</p>
                    </div>
                    <div>
                        <label style="font-weight:700;color:#374151;display:block;margin-bottom:6px;">Lý do chấm hộ <span style="color:#ef4444">*</span></label>
                        <textarea id="override-lydo" rows="3" placeholder="VD: Máy tablet mất nguồn, nhân viên làm ngoài văn phòng có phân công..."
                            style="width:100%;padding:10px 14px;border:1px solid #d1d5db;border-radius:10px;font-size:.95rem;resize:vertical;box-sizing:border-box;"></textarea>
                    </div>
                    <div>
                        <label style="font-weight:700;color:#374151;display:block;margin-bottom:6px;">Ghi chú thêm (tuỳ chọn)</label>
                        <textarea id="override-ghichu" rows="2" placeholder="Ghi chú thêm nếu có (bằng chứng, minh chứng đính kèm bên ngoài hệ thống...)"
                            style="width:100%;padding:10px 14px;border:1px solid #d1d5db;border-radius:10px;font-size:.95rem;resize:vertical;box-sizing:border-box;"></textarea>
                    </div>
                </div>
            </div>
            ...
            <!-- Bước 3: Xác nhận -->
            <div class="override-step-pane" id="override-pane-3">
                <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:14px;padding:18px;margin-bottom:16px;">
                    <h4 style="margin:0 0 12px;color:#92400e;display:flex;align-items:center;gap:8px;"><i class="fas fa-exclamation-triangle"></i> Xác nhận chấm công hộ</h4>
                    <div id="override-confirm-detail" style="display:grid;gap:8px;font-size:.9rem;"></div>
                </div>
                <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:14px;font-size:.85rem;color:#166534;">
                    <i class="fas fa-shield-alt" style="margin-right:6px;"></i>
                    Thao tác này sẽ được <strong>ghi nhận lịch sử đầy đủ</strong>: tên HR, thời điểm thực hiện, lý do chấm hộ. 
                    Dữ liệu không thể xóa để đảm bảo tính minh bạch.
                </div>
            </div>
        </div>
        <div class="override-footer">
            <button type="button" class="btn btn-secondary btn-sm" id="override-btn-prev" style="display:none;">← Quay lại</button>
            <button type="button" class="btn btn-secondary btn-sm" id="override-btn-cancel">Huỷ</button>
            <button type="button" class="btn btn-sm" id="override-btn-next" style="background:#f59e0b;color:#fff;border:none;">Tiếp theo →</button>
            <button type="button" class="btn btn-sm" id="override-btn-submit" style="background:#16a34a;color:#fff;border:none;display:none;"><i class="fas fa-check"></i> Xác nhận chấm hộ</button>
        </div>
    </div>
</div>
...
<script>
    ...
    function calcWorkHours(checkIn, checkOut) {
        ...
        return Math.round((outTs - inTs) / 36000) / 100;
    }
    ...
    function renderEmployeeDetailModal(maND, hoTen, phongBan, dailyBreakdown, monthKey) {
        ...
        if (title) title.textContent = 'Chi tiết chấm công: ' + (hoTen || ('#' + maND));
        if (subtitle) subtitle.textContent = 'Phòng ban: ' + (phongBan || '-') + ' | Tháng: ' + monthKey;

        var parts = (monthKey || '').split('-');
        var year  = parseInt(parts[0], 10);
        var month = parseInt(parts[1], 10);
        if (!year || !month) { 
            if (tbody) tbody.innerHTML = '<tr><td colspan="8" class="empty-state">Không xác định được tháng.</td></tr>'; 
        ...
        var lateDays = 0;
        var rows = [];
        var bd = dailyBreakdown || {};

        for (var d = 1; d <= lastDay; d++) {
            var dateStr = year + '-' + (month < 10 ? '0' : '') + month + '-' + (d < 10 ? '0' : '') + d;
            var dayData = bd[dateStr] || null;
            ...
            var lateMinutes = 0;
            if (checkIn) {
                var shiftStart = dayData && dayData.shift_start ? dayData.shift_start : '08:00:00';
                var inTs = new Date(checkIn.replace(' ', 'T')).getTime();
                var shiftStartTs = new Date(checkIn.substring(0, 10) + 'T' + shiftStart).getTime();
                if (!isNaN(inTs) && !isNaN(shiftStartTs) && inTs > shiftStartTs + 60000) {
    ...
    window.showEmployeeDetailById = function(maND, monthKey) {
        var breakdown = (window._empBreakdowns && window._empBreakdowns[maND]) ? window._empBreakdowns[maND] : {};
        var meta = (window._empMeta && window._empMeta[maND]) ? window._empMeta[maND] : {};
        var month = monthKey;
        if (!month) {
            var filterForm = document.getElementById('payroll-filter-form');
            var mInput = filterForm ? filterForm.querySelector('[name="month"]') : null;
            month = mInput ? mInput.value : '';
        }
        renderEmployeeDetailModal(maND, meta.hoTen || '', meta.phongBan || '-', breakdown, month);
    };
    ...
    window.showDailyDetail = function(name, date, checkIn, checkOut) {
        ...
        if (!modal) return;
        var nameEl = document.getElementById('daily-detail-name');
        var dateEl = document.getElementById('daily-detail-date-label');
        var inEl = document.getElementById('daily-detail-in');
        var outEl = document.getElementById('daily-detail-out');
        if (nameEl) nameEl.textContent = name;
        if (dateEl) dateEl.textContent = 'Ngày: ' + date;
        if (inEl) inEl.textContent = checkIn ? checkIn.substring(11, 19) : '--:--';
        if (outEl) outEl.textContent = checkOut ? checkOut.substring(11, 19) : '--:--';
        modal.classList.add('open');
    };
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Đóng modal chi tiết nhân viên khi click ngoài
...
<script>
// ===== CHẤM CÔNG HỘ (HR Override) =====
document.addEventListener('DOMContentLoaded', function () {
    var modal        = document.getElementById('hrOverrideModal');
    var btnOpenCard  = document.getElementById('btn-open-override-modal');
    var btnOpenTab   = document.getElementById('btn-open-override-modal-2');
    var btnClose     = document.getElementById('override-modal-close');
    var btnCancel    = document.getElementById('override-btn-cancel');
    var btnNext      = document.getElementById('override-btn-next');
    var btnPrev      = document.getElementById('override-btn-prev');
    var btnSubmit    = document.getElementById('override-btn-submit');
    var overrideTab  = document.querySelector('.sub-tab[data-tab="tab-chamconghothay"]');
    var monthInput   = document.querySelector('#payroll-filter-form [name="month"]');

    var currentStep = 1;
    var overrideHistoryLoaded = false;

    function getMonth() {
        return monthInput ? monthInput.value : '<?= htmlspecialchars($selectedMonth) ?>';
    }
    ...
    function openModal() {
        ...
        modal.setAttribute('aria-hidden', 'false');
        // Reset fields
        document.getElementById('override-nv-select').value = '';
        document.getElementById('override-nv-preview').style.display = 'none';
        document.getElementById('override-ngay').value = '';
        document.getElementById('override-lydo').value = '';
        document.getElementById('override-ghichu').value = '';
    }
    ...
    function renderStep() {
        // Step buttons
        document.querySelectorAll('.override-step-btn').forEach(function(btn) {
        ...
        // Panes
        document.querySelectorAll('.override-step-pane').forEach(function(pane) {
            pane.classList.remove('active');
        });
        var active = document.getElementById('override-pane-' + currentStep);
        ...
        if (currentStep === 3) buildConfirm();
    }

    function buildConfirm() {
        var sel = document.getElementById('override-nv-select');
        var opt = sel.options[sel.selectedIndex];
        var name = opt ? opt.getAttribute('data-name') : '';
        var dept = opt ? opt.getAttribute('data-dept') : '';
        var ngay = document.getElementById('override-ngay').value;
        var lydo = document.getElementById('override-lydo').value;
        var ghichu = document.getElementById('override-ghichu').value;

        document.getElementById('override-confirm-detail').innerHTML =
            '<div><strong style="color:#64748b;">Nhân viên:</strong> ' + escH(name) + ' (' + escH(dept) + ')</div>' +
            '<div><strong style="color:#64748b;">Ngày chấm hộ:</strong> ' + escH(ngay) + '</div>' +
            '<div><strong style="color:#64748b;">Giờ vào:</strong> 08:00 | <strong style="color:#64748b;">Giờ ra:</strong> 17:00</div>' +
            '<div><strong style="color:#64748b;">Lý do:</strong> ' + escH(lydo) + '</div>' +
            (ghichu ? '<div><strong style="color:#64748b;">Ghi chú:</strong> ' + escH(ghichu) + '</div>' : '');
    }

    function validateStep() {
        if (currentStep === 1) {
            var sel = document.getElementById('override-nv-select');
            if (!sel.value) { alert('Vui lòng chọn nhân viên!'); return false; }
        }
        if (currentStep === 2) {
            var ngay = document.getElementById('override-ngay').value;
            var lydo = document.getElementById('override-lydo').value.trim();
            if (!ngay)  { alert('Vui lòng chọn ngày chấm hộ!'); return false; }
            if (!lydo)  { alert('Vui lòng nhập lý do chấm hộ!'); return false; }
            if (ngay > new Date().toISOString().slice(0,10)) {
                alert('Không thể chấm công hộ cho ngày trong tương lai!'); return false;
            }
        }
        return true;
    }
    ...
    // NV select preview
    document.getElementById('override-nv-select').addEventListener('change', function() {
        var opt = this.options[this.selectedIndex];
        var preview = document.getElementById('override-nv-preview');
        if (this.value) {
            document.getElementById('override-nv-name').textContent = opt.getAttribute('data-name') || '';
            document.getElementById('override-nv-dept').textContent = opt.getAttribute('data-dept') || 'N/A';
    ...
    if (btnOpenCard) btnOpenCard.addEventListener('click', openModal);
    ...
    if (btnSubmit) {
        btnSubmit.addEventListener('click', function() {
            ...
            var ngay  = document.getElementById('override-ngay').value;
            var lydo  = document.getElementById('override-lydo').value.trim();
            var ghichu = document.getElementById('override-ghichu').value.trim();
            ...
            .then(function(json) {
                if (json.success) {
                    closeModal();
                    alert('✅ ' + (json.message || 'Chấm công hộ thành công!'));
                    // Refresh history if tab already loaded
                    overrideHistoryLoaded = false;
                    loadOverrideHistory();
                    // Update metric badge
                    var mc = document.getElementById('metric-override');
                    var tb = document.getElementById('override-tab-badge');
                    if (mc) {
                        var cur = parseInt(mc.textContent) || 0;
                        mc.textContent = (cur + 1) + ' lần';
                    }
                    if (tb) {
                        var cb = parseInt(tb.textContent) || 0;
                        tb.textContent = cb + 1;
                    }
                } else {
                    alert('❌ ' + (json.message || 'Không thể chấm công hộ.'));
                }
            })
            .catch(function() { alert('Lỗi kết nối máy chủ.'); })
            .finally(function() {
                btnSubmit.disabled = false;
                btnSubmit.innerHTML = '<i class="fas fa-check"></i> Xác nhận chấm hộ';
            });
        });
    }
    ...
    function loadOverrideHistory() {
        ...
        var loading = document.getElementById('override-history-loading');
        if (!tbody) return;
        if (loading) loading.style.display = '';
        tbody.innerHTML = '<tr><td colspan="9" class="empty-state">Đang tải...</td></tr>';

        var month = getMonth();
        if (document.getElementById('override-month-label')) {
            document.getElementById('override-month-label').textContent = month;
        }

        fetch('index.php?page=hr-api-override-history&month=' + encodeURIComponent(month), {
            headers: { 'Accept': 'application/json' }
        })
        .then(function(r) { return r.json(); })
        .then(function(json) {
            ...
            if (!json.success) {
                tbody.innerHTML = '<tr><td colspan="9" class="empty-state">' + escH(json.message || 'Lỗi tải dữ liệu') + '</td></tr>';
                return;
            }
            var rows = json.data || [];
            if (!rows.length) {
                tbody.innerHTML = '<tr><td colspan="9" class="empty-state">Chưa có lần chấm công hộ nào trong tháng ' + escH(month) + '</td></tr>';
                return;
            }
            tbody.innerHTML = rows.map(function(row, i) {
                var gioVao = row.gioVao ? row.gioVao.substring(11,16) : '--:--';
                var gioRa  = row.gioRa  ? row.gioRa.substring(11,16)  : '--:--';
                var thoiDiem = row.ngayTao ? row.ngayTao.replace('T',' ').substring(0,19) : '-';
                return '<tr>' +
                    '<td>' + (i+1) + '</td>' +
                    '<td><strong>' + escH(row.tenNhanVien || ('#NV'+row.maNguoiDuocChamHo)) + '</strong></td>' +
                    '<td>' + escH(row.phongBan || '-') + '</td>' +
                    '<td><strong>' + escH(row.ngayChamHo) + '</strong></td>' +
                    '<td>' + escH(gioVao) + '</td>' +
                    '<td>' + escH(gioRa)  + '</td>' +
                    '<td style="max-width:220px;white-space:normal;">' + escH(row.lyDo) + '</td>' +
                    '<td><span style="background:#dbeafe;color:#1d4ed8;border-radius:6px;padding:2px 8px;font-size:.8rem;font-weight:700;">' + escH(row.tenHR || 'HR') + '</span></td>' +
                    '<td style="color:#64748b;font-size:.82rem;">' + escH(thoiDiem) + '</td>' +
                    '</tr>';
            }).join('');
            overrideHistoryLoaded = true;

            // Update metric & badge
            var mc = document.getElementById('metric-override');
            var tb = document.getElementById('override-tab-badge');
            if (mc) mc.textContent = rows.length + ' lần';
            if (tb) tb.textContent = rows.length;
        })
        .catch(function() {
            if (loading) loading.style.display = 'none';
            tbody.innerHTML = '<tr><td colspan="9" class="empty-state">Lỗi kết nối máy chủ.</td></tr>';
        });
    }

    // Load when tab is clicked
    if (overrideTab) {
        overrideTab.addEventListener('click', function() {
            if (!overrideHistoryLoaded) loadOverrideHistory();
        });
    }
    ...
    if (monthInput) {
        monthInput.addEventListener('change', function() {
            overrideHistoryLoaded = false;
            // If override tab is active, reload immediately
            var activeTab = document.querySelector('.sub-tab.active[data-tab="tab-chamconghothay"]');
            if (activeTab) loadOverrideHistory();
        });
    }
});
</script>
